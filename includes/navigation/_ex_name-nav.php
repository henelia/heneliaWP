<ul>
	<?php
		...
		$name_menu_config = array(
			...
			'items_wrap'      => '%3$s',
			'depth'           => 1,
			'walker'          => new Name_Walker()
		);
		wp_nav_menu( $name_menu_config );
	?>
</ul>

<?php
	// Walker du name nav
	class Name_Walker extends Walker_Nav_Menu {
	...
	function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		$class_names = 'header-nav-list name-nav-list_l' . $display_depth . ' name-nav-list reset-list no-first';
		// construction du <ul>
	...
	function start_el(  &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		// classes communes et en fonction de la profondeur
		$depth_class_names = 'header-nav-item name-nav-item name-nav-item_l' . $display_depth;
		// classes wordpress
		...
		$attributes .= ' class="header-nav-link name-nav-link name-nav-link_l'.$display_depth.'"';
		$item_output = sprintf( '%1$s<a%2$s>%3$s%4$s%5$s%6$s%7$s</a>%8$s',
			$args->before,
			$attributes,
			$args->link_before,
			'<span class="header-nav-link-inner name-nav-link-inner name-nav-link-inner_l' . $display_depth . '">',
